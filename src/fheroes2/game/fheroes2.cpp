...
int powerCallback(int arg1, int arg2, int arg3, void *common)
{
    //it just crashes w/o cout here..
    COUT(arg3);
    //just reinit on all events.. because why not
    StreamFile::reinitAllFiles();
}
#endif
...
#if !defined(ANDROID) && !defined(VITA)
	try
#endif
	{
	    std::atexit(SDL::Quit);
...
void LoadZLogo(void)
{
#if defined(BUILD_RELEASE) && !defined(VITA)
