...
bool Settings::Read(const std::string & filename)
{
    ...
    if(config.Exists("vita_touchcontrol_speed"))
    {
	int vita_touchcontrol_speed = config.IntParams("vita_touchcontrol_speed");
	if (vita_touchcontrol_speed > 100) vita_touchcontrol_speed = 100;
        if (vita_touchcontrol_speed < 0) vita_touchcontrol_speed = 0;
        LocalEvent::Get().SetVitaTouchControlSpeed(vita_touchcontrol_speed);
    }
#endif
...
ListFiles Settings::GetListFiles(const std::string & prefix, const std::string & filter)
{
    ...
    //uppercase dir fix..
    else if(prefix.size() && System::IsDirectory(StringUpper(prefix)))
	res.ReadDir(prefix, filter, false);
