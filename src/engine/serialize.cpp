...
StreamBase & StreamBase::operator>> (int & v)
{
    v = get32();
    return *this;
}
...
StreamBase & StreamBase::operator<< (const int & v)
{
    put32(static_cast<s32>(v));
    return *this;
}
...
//dirtiest hack known to the mankind to fight file handlers on vita suspend
#ifdef VITA
std::list<StreamFile*> StreamFile::openedFiles;

void StreamFile::reinitFile(void)
{
    if (!rw)
        return;
    ...
    SDL_RWclose(rw);
    rw = NULL;
    rw = SDL_RWFromFile(filename.c_str(), reinterpret_cast<const char*>(filemode));
    SDL_RWseek(rw, pos, RW_SEEK_SET);
}

void StreamFile::reinitAllFiles(void)
{
    for ( StreamFile* openedFile : openedFiles ) 
    {
        openedFile->reinitFile();
    }
}
#endif
...
#ifdef VITA
    filename = fn;
    filemode = const_cast<char*>(mode);
    openedFiles.push_back(this);
#endif
